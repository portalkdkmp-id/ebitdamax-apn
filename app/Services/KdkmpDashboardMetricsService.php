...
class KdkmpDashboardMetricsService
{
    ...
    /**
     * @return array{actual_cost: string, total_duration: string}
     */
     ...
     * @return Collection<int, array{actual_cost: string, total_duration: string}>
     */
    public function forUsers(iterable $userIds, CarbonImmutable $businessDate): Collection
    {
        ...
        $durationMinutesByUser = TaskReport::query()
            ->select('user_id')
            ...
            ->groupBy('user_id')
            ->pluck('total_duration_minutes', 'user_id');
        ...
        return $ids->mapWithKeys(function (int $userId) use ($durationMinutesByUser, $expenseTotalsByUser): array {
            $durationMinutes = (int) $durationMinutesByUser->get($userId, 0);
